...
async def reminder_edit_text_start(callback: types.CallbackQuery):
    ...
    await ReminderStates.edit_reminder_id.set()
    await callback.answer()
...
async def reminder_edit_date(message: types.Message, state: FSMContext):
    ...
    await ReminderStates.edit_time.set()
    await message.answer("🕐 Выбери час:", reply_markup=get_reminder_hour_buttons())

@dp.message_handler(state=ReminderStates.edit_time)
...
async def reminder_edit_minute(message: types.Message, state: FSMContext):
    ...
    reminder_id = data.get("edit_reminder_id")
    if reminder_id:
        new_time = f"{data['hour']:02d}:{message.text}"
        # Здесь можно добавить метод обновления времени напоминания
        await message.answer("✅ Время напоминания обновлено", reply_markup=get_reminder_menu())
    else:
        await message.answer("❌ Ошибка", reply_markup=get_reminder_menu())
    await state.finish()
