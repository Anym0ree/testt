...
class AIAdvisor:
    """Класс для получения AI-советов на основе данных пользователя с поддержкой диалога."""
    ...
    async def get_advice(self, user_id: int, user_question: Optional[str] = None) -> str:
        ...
        if not self.api_key:
            return ("❌ AI-модуль не настроен.\n"
                    "Добавьте API-ключ OpenAI в config.py (переменная OPENAI_API_KEY).\n"
        ...
        if user_question:
            ...
            # Если это первый запрос, добавим просьбу дать общий анализ
